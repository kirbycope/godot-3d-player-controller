...
func _input(event: InputEvent) -> void:
	...
	if player != null:
		...
		if event.is_action_pressed("button_13"):
			# Get the item
			var item = player.bone_attachement_left_hand.get_children()[0]
			# Remove the item from the player
			player.bone_attachement_left_hand.remove_child(item)
...
func _on_area_3d_body_entered(body: Node3D) -> void:
	...
	if body is CharacterBody3D and body.is_in_group("Player"):
		...
		if !body.is_skateboarding:
			...
			body.bone_attachement_left_hand.add_child(instance)
